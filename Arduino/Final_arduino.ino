#include <BarreraControl.h>  // Nueva librería
...
// Crear objeto de la librería de barrera
BarreraControl controlBarrera(&barrera, 4);
...
void setup() {
  ...
  // Configurar la barrera usando la librería
  controlBarrera.configurar(2, 0, 90);
}
...
void leerMensajesSerial() {
  ...
  if (mensaje.startsWith("Lugar:")) {
    ...
    if (lugar > 0) {
      ...
      controlBarrera.abrir();  // Abre, espera 1 seg y cierra
    } else {
  ...
  if (mensaje.startsWith("NoSocio:")) {
    ...
    controlBarrera.moverNegacion();  // Usar librería
    volverAPantallaActual();
    return;
  }
